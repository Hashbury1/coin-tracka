...
async def enrich_tick(session: ClientSession, tick: dict) -> dict:
    """
    Best-effort enrichment with holder concentration (rug-pull risk signal)
    and social mention velocity (buzz signal). Only called for the top N
    tokens by liquidity per cycle — see poll_once — since these are
    per-token API calls against rate-limited third-party services.

    Never raises: every branch here is designed so a missing key, a 429,
    or an unexpected response shape just leaves the corresponding field
    as None, and scoring treats that dimension as "unavailable" rather
    than failing.
    """
...
async def poll_once(session: ClientSession, redis_client: redis.Redis) -> None:
    ...
    # enrich_tick mutates each dict in place, and sorted() below reorders
    # references rather than copying, so these mutations are reflected
    # directly in all_ticks without needing to merge anything back.
