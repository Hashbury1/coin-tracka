...
This runs the exact same topology you'd use in a real deployment â real
Deployments, Services, Ingress, and a real Prometheus Operator â just on a
...
## 3. Install ingress-nginx (kind-specific manifest â uses hostPort, not LoadBalancer)
...
kind runs its own containerd inside the cluster nodes â it can't see images
...
(Adjust the image names above if your actual built tags differ â check
...
initContainer waiting on Redis/TimescaleDB â check with:
```bash
...
This takes a few minutes â it's installing Prometheus, Alertmanager,
...
â it's a local cluster, this is fine here, would never be fine anywhere
...
Open `http://localhost:9090/targets` â you should see `ingestion`,
...
label doesn't match your Helm release name â see the comment in
...
This deletes everything â cluster, PVC-backed data, all of it â cleanly,
since it never touched anything outside the kind container itself.

## What this setup deliberately does NOT do

- No TLS on the Ingress (no cert-manager) â fine locally, would be a real
  gap in production
- Grafana admin password is a hardcoded `admin`/`admin` â acceptable only
  because this never leaves your machine
- Single replica everywhere â no actual HA being tested, just topology
- Alertmanager has no real notification channel wired up (no Slack/email) â
